...
func (c *Client) buildFirecrackerConfig(ctx context.Context, vmID string, config *metaldv1.VmConfig, networkInfo *network.VMNetwork, preparedPaths map[string]string) sdk.Config {
	...
	// Add drives
	c.addDrivesToConfig(&cfg, config, jailerRoot)
...
// addDrivesToConfig adds storage drives to the Firecracker configuration
func (c *Client) addDrivesToConfig(cfg *sdk.Config, config *metaldv1.VmConfig, jailerRoot string) {
	...
	for i, disk := range config.GetStorage() {
		...
		// Use standardized filename for root devices
